...
> **Keep in sync with CLAUDE.md.** These two files MUST stay identical except for the agent-name line (§1) and the `pulse-ocpp-engine` cross-reference (§ "pulse-ocpp-engine"). If you edit one, edit the other. Consider replacing both with a symlink to a single shared file if your tooling supports it.
