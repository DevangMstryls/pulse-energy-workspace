...
A Headroom proxy is running at `http://127.0.0.1:8787` and transparently compresses all model traffic. In addition, three MCP tools are available for on-demand use:
...
2. Render a `### Headroom stats` block immediately AFTER the Work Summary table and BEFORE any "next steps" list, in this format:
...
3. If `headroom_stats` is unavailable or errors, note that in the Work Summary `Additional Notes` column — never fabricate numbers.
4. For purely informational/Q&A turns where no Work Summary is produced, the stats block is optional and should be included only if the user asks.
