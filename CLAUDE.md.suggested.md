...
> **Keep in sync with AGENTS.md.** These two files MUST stay identical except for the agent-name line (§1) and the `pulse-ocpp-engine` cross-reference (§ "pulse-ocpp-engine"). If you edit one, edit the other. Consider replacing both with a symlink to a single shared file if your tooling supports it.

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Table of Contents

1. [Quickstart](#quickstart) — clone, bootstrap, run
2. [When in Doubt (Decision Tree)](#when-in-doubt-decision-tree)
3. [Workspace Overview](#workspace-overview) — projects table, submodules
4. [Project Guides](#project-guides) — per-project commands + architecture
5. [Cross-Project Patterns](#cross-project-patterns)
6. [Code Conventions](#code-conventions) — naming, API shape, errors, logging, testing
7. [Git Workflow](#git-workflow) — branching, commits, PRs, submodules
8. [Deployment & GitOps (ArgoCD)](#deployment--gitops-argocd)
9. [Secrets & Restricted File Access](#secrets--restricted-file-access)
10. [Documentation](#documentation) — pulse-energy-docs mirror, JSDoc
11. [Accuracy & Agent Behavior](#accuracy--agent-behavior) — response formatting, command reporting, work summary
12. [Headroom (Context Compression)](#headroom-context-compression)

---

## Quickstart

First-time setup for any project in this workspace:

```bash
# 1. Clone the specific project repo (each project is its own git repo)
git clone <project-remote> && cd <project-dir>

# 2. Initialize and update submodules (central-atoms, beckn-atoms, pulse-node-utils-lib)
git submodule update --init --recursive

# 3. Install dependencies
npm install

# 4. For pulse-central only — start local Postgres via Docker
docker compose up -d

# 5. For pulse-central only — generate Prisma client and run migrations
npm run prisma:setup
npx prisma migrate deploy

# 6. Copy the example env file and fill in local values
cp .dev.env.example .dev.env   # NEVER commit .dev.env

# 7. Start the dev server
npm run start:dev               # pulse-central
npm run dev                     # central-console
```

Node.js 22+ is required for all backend projects. Use `nvm use` if a `.nvmrc` is present.

---

## When in Doubt (Decision Tree)

If unsure where to look or what to touch, follow this:

1. Backend API / billing / OCPP / OCPI / fleets → [pulse-central](#pulse-central-main-backend)
2. Frontend admin dashboard → [central-console](#central-console-frontend-dashboard)
3. OCPP WebSocket server (dedicated) → [pulse-ocpp-engine](#pulse-ocpp-engine)
4. Mobile app → [InstaCharge](#instacharge-mobile-app)
5. Public website → [pulse-web](#pulse-web-public-website)
6. Lightweight PWA → [pulse-energy-lite](#pulse-energy-lite-pwa)
7. Video streaming → [pulse-streamer](#pulse-streamer)
8. Data analytics / SQL → [pulse-dataverse](#pulse-dataverse)
9. AI / LLM / embeddings → [pulse-ai](#pulse-ai)
10. Shared types / enums / constants → `central-atoms` submodule (always check here before creating new types)
11. Deploy / ArgoCD / sync / health → load the `pulse-argocd` skill
12. Shared observability / Kafka / HTTP server utils → `pulse-node-utils-lib` submodule
...
> **Verify versions**: Tech-stack versions in this table may lag behind actual `package.json` values. Run `cat package.json | jq '.dependencies | {react, next, "@mui/material"}'` inside a project to confirm before relying on a version number.
...
**Submodule rules** (see [Git Workflow — Submodules](#submodules) for details):

1. Always check `central-atoms` before creating new types — types are organized by entity domain.
2. Editing a submodule file in-place does NOT update the parent repo. You must commit in the submodule, then bump the submodule pointer in the parent repo. Otherwise your changes are silently lost.
3. To update a submodule to its latest: `git submodule update --remote src/central-atoms`, then commit the pointer bump in the parent.
...
**Docker**: `docker-compose.yml` provides PostgreSQL 14 on port 5432 for local dev. Refer to the compose file for current local credentials — do not hardcode them here.
...
See `pulse-ocpp-engine/CLAUDE.md` for detailed architecture. Key points:
...
### InstaCharge (Mobile App)

**Stack**: React Native 0.76 + Expo 52, Redux

**Commands** (run from `InstaCharge/`):

```bash
npm install               # Install deps
npx expo start            # Start Expo dev server (QR code for device)
npx expo start --ios      # iOS simulator
npx expo start --android  # Android emulator
npm run build             # Expo build (EAS Build for native binaries)
npm test                  # Jest tests
```

**Architecture**: Redux for state. Expo for build/OTA updates. Consumer-facing — charging station discovery, QR-start charging, payments, transaction history.

---

### pulse-web (Public Website)

**Stack**: Next.js 10, React 16, MUI

**Commands** (run from `pulse-web/`):
...
**Architecture**: Next.js SSR/SSG. Public marketing site + charging station locator. Google Maps API for station map.

---

### pulse-energy-lite (PWA)

**Stack**: React 18, Create React App

**Commands** (run from `pulse-energy-lite/`):
...
**Architecture**: Lightweight cross-platform PWA. Subset of central-console functionality for end users.

---

### pulse-streamer

**Stack**: Node.js, TypeScript, ffmpeg, Google Cloud Storage

**Commands** (run from `pulse-streamer/`):
...
**Architecture**: Converts RTSP camera feeds to HLS streams for browser playback. Outputs to GCS. ffmpeg child process management per stream.

---

### pulse-dataverse

**Stack**: Node.js, TypeScript, Prisma, Kafka

**Commands** (run from `pulse-dataverse/`):
...
**Architecture**: Data analytics service. Consumes Kafka events, executes SQL queries against analytics databases, exposes query API. Prisma for metadata storage.

---

### pulse-ai

**Stack**: Node.js, TypeScript, LangChain, ChromaDB, OpenAI

**Commands** (run from `pulse-ai/`):
...
**Architecture**: LLM service. LangChain for orchestration, ChromaDB for vector embeddings, OpenAI for generation. Used for chatbot, smart search, and document Q&A.

---

### pulse-central-alpha

Alpha/staging variant of pulse-central. Same commands and architecture — see [pulse-central](#pulse-central-main-backend). Deployed to the alpha/staging environment for pre-prod validation.
...
- **Environment files**: `.dev.env`, `.stg.env`, `.prod.env` loaded via `env-cmd`. Copy from `.dev.env.example`; never commit real env files.
...
All REST handlers respond via `returnResponse()`. The standard response envelope:
...
- Success → HTTP 2xx, `"status": "success"`, payload in `data`.
- Error → appropriate HTTP 4xx/5xx, `"status": "error"`, details in `error`.
- Pagination: `data` contains `{ items: [...], page: N, pageSize: M, total: T }` for list endpoints.
- Never return raw Prisma errors to the client — catch, map to a stable error code, and log the original.

### Error Handling

- `db-services` throw on failure; `services` catch and translate to domain errors; handlers catch and format the response.
- Never swallow errors silently — at minimum `logger.error({ err, context })` before returning a generic error to the client.
- Use typed error classes where they exist; create one per domain if adding a new error category.

### Logging

- **Winston** is the logger across all backends.
- Log levels: `error` (failures, exceptions), `warn` (degraded but functional), `info` (lifecycle events), `debug` (verbose, dev only).
- Always include structured context: `logger.error({ err, userId, chargePointId, action: 'startCharging' })`.
- Never log secrets, tokens, PII, or full payment payloads.

### Testing

- **Jest** across all projects. E2E tests run with a single worker (`npm run test:e2e`).
- Test files: `*.test.ts` / `*.spec.ts` co-located with the module or in a `__tests__/` directory.
- New service or handler logic MUST include unit tests. New DbService methods MUST have tests for the happy path and at least one error case.
- Run `npm test` before requesting review — do not push broken tests.

### Formatting

- **pulse-central**: Prettier with 4-space tabs, 120 char width, trailing commas, single quotes. ESLint with Airbnb + TypeScript config.
- **central-console**: Prettier with 4-space tabs, 140 char width, single quotes, no trailing commas. ESLint with TypeScript.
- Other projects: check the local `.prettierrc` / `.eslintrc` — do not assume.
...
1. Default integration branch is `develop` unless a repo's active default is `main`, `master`, or `staging` (check `git rev-parse --abbrev-ref HEAD` and `git symbolic-ref refs/remotes/origin/HEAD`).
2. Feature branches: `feature/<short-description>` or `fix/<short-description>`.
...
3. Pull the latest code for the current branch with `git pull --ff-only` before committing. If the pull fails (non-fast-forward), fetch with `git fetch origin <branch>` and rebase or ask the user before proceeding.
4. Record the actual branch used in the Work Summary `Branch` column.
...
Types: `feat`, `fix`, `chore`, `refactor`, `docs`, `test`, `ci`, `perf`. Scope is optional but encouraged (e.g., `feat(billing): ...`).

### Pull Requests

1. PR title should match the commit subject (without the type prefix is fine for human-readable title).
2. PR description: what changed, why, how to test, any migrations/env/feature flags, related ticket.
3. Link related tickets in the description.
4. Do not merge your own PR without review unless explicitly authorized.

### Submodules

1. **Editing submodule files**: Commit inside the submodule first (`cd src/central-atoms && git add . && git commit`). Then bump the pointer in the parent repo (`cd ../.. && git add src/central-atoms && git commit -m "chore: bump central-atoms"`). If you skip the parent bump, your submodule changes are invisible to anyone cloning fresh.
2. **Updating to latest**: `git submodule update --remote src/central-atoms` → review the diff → commit the pointer bump.
3. **Cloning fresh**: `git clone --recurse-submodules <remote>` or `git submodule update --init --recursive` after clone.
4. **Never edit submodule files without understanding this workflow** — silent data loss is the #1 submodule mistake.
...
1. Do not run Prisma migrations against staging or production databases from a local machine — use the CI/CD pipeline or ArgoCD.
2. Do not edit submodule files without bumping the parent pointer (see [Git Workflow — Submodules](#submodules)).
3. Do not add new top-level directories under the workspace root without explicit approval.
4. Do not deploy to production on Fridays or before weekends unless explicitly authorized.
5. Do not log secrets, tokens, PII, or full payment payloads.
6. Do not push broken tests — `npm test` must pass before requesting review.
...
6. The `pulse-energy-docs` default branch is `master` — this overrides the workspace-wide `develop` default per the per-repo override clause in [Git Workflow — Branching](#branching).
...
- Each property of a type or interface should have a JSDoc comment describing what it is, its purpose, and an example of the expected values.

---

## Accuracy & Agent Behavior
